...
CONFIG.TOOLTIP.SAKUZU = {
  ...
  "LINE_GEODESIC": "地球上の2点間の最短コースです。地震学地図上では曲線になります。",
  "LINE_STRAIGHT": "経線に対して一定の角度を保って進むコースです。地震学地図上では直線になります。"
};
...
GSITEXT.SHARE.DIALOG_BUILTIN_MESSAGE = '次のタグをHTMLファイルに加えることで、地震学地図をウェブサイトに埋め込むことができます。' + '<div style="font-size:85%;">※作図結果は反映されません。作図結果は、作図・ファイル機能を使ってファイルに保存して共有することをおすすめいたします。</div>';
...
GSITEXT.SHARE.MAIL_SUBJECT = '地震学地図';
...
GSITEXT.MAPTOIMAGE.WINDOW_MSG2 = '※保存したファイルは、各種利用規約に従ってご利用ください。<br>' +
...
GSITEXT.EVAC.CONFIRMTOP = '地震学地図及び重ねるハザードマップに掲載されている指定緊急避難場所データ（以下、「本データ」といいます）を利用される場合は、<a href="https://www.gsi.go.jp/kikakuchousei/kikakuchousei40182.html" target="blank">国土地理院コンテンツ利用規約</a>のほか、以下のご利用上の注意をご確認いただき、内容に同意された場合のみご利用ください。';
...
GSITEXT.EVAC.CONFIRMITEM2 = '本データを、ダウンロードや印刷等を行い本サーバ外で利用される場合は、本データの更新にあわせて最新の情報をご利用ください（参照：<a href="https://www.gsi.go.jp/bousaichiri/hinanbasho.html#info2" target="blank">市町村別公開日・更新日一覧</a>）。';
GSITEXT.EVAC.CONFIRMITEM3 = '指定緊急避難場所は、災害種別ごとに指定されています。本データをダウンロードや印刷等を行い本サーバ外で利用される場合、指定された災害種別を利用者が正確に理解できるよう、十分にご留意ください。';
